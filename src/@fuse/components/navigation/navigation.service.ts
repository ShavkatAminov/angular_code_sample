...
export class FuseNavigationService
{
    ...
    getPathOfActiveElement( link: string, navigations: FuseNavigationItem[]): number[] {
        const path = []

        if (this.findActiveElement(path, 0, navigations, link) === undefined
            || this.findActiveElement(path, 0, navigations, link) === null ){
            path.length = 0
        }
        return path
    }

    findActiveElement(path: number[], depth: number, items: FuseNavigationItem[], target: string):void|number {
        for (const [idx, node ] of items.entries()){
            ...
            if (
                ...
                node.type &&
                node.type === "collapsable" &&
                node.children &&
                Array.isArray(node.children) &&
                node.children.length > 0
            ) {
                const foundIdx = this.findActiveElement(
                    path,
                    depth + 1,
                    node.children,
                    target
